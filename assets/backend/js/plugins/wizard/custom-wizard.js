    ...
    $.fn.Wizard = function(options) {
        ...
        var Plugin = {
            ...
            init: function(options) {
                //== Elements
                wizard.steps = wizard.find('.wizard_step');
                wizard.progress = wizard.find('.wizard-content-progress .progress-bar'); 
                ...
                wizard.options = $.extend(true, {}, $.fn.Wizard.defaults, options);
            ...
            build: function() {
                ...
                wizard.find('.wizard_step a.wizard_step-number').on('click', function() {
                    var step = $(this).parents('.wizard_step');
                    var num;
                    $(this).parents('.wizard_steps').find('.wizard_step').each(function(index) {
                        if (step.is( $(this) )) {
                            num = (index + 1);
                            return;
                        }
            ...
            updateUI: function(argument) {
                ...
                wizard.find('.wizard_step').removeClass('wizard_step-current').removeClass('wizard_step-done');
                for (var i = 1; i < wizard.currentStep; i++) {
                    wizard.find('.wizard_step').eq(i - 1).addClass('wizard_step-done');
                }
                wizard.find('.wizard_step').eq(wizard.currentStep - 1).addClass('wizard_step-current');
            },
            ...
            setStepClass: function() {
                if (Plugin.isLastStep()) {
                    element.addClass('wizard-step-last');
                } else {
                    element.removeClass('wizard-step-last');
                }

                if (Plugin.isFirstStep()) {
                    element.addClass('wizard-step-first');
                } else {
                    element.removeClass('wizard-step-first');
                }

                if (Plugin.isBetweenStep()) {
                    element.addClass('wizard-step-between');
                } else {
                    element.removeClass('wizard-step-between');
                }
            },
            ...
            updateProgress: function() {
                ...
                //== Update progress
                if (element.hasClass('wizard-1')) {
                    var width = 100 * ((wizard.currentStep) / (wizard.totalSteps));
                    var offset = element.find('.wizard_step-number').width();
                    wizard.progress.css('width', 'calc(' + width + '% + ' + (offset / 2)  + 'px)');
                } else if (element.hasClass('wizard-2')) {
                    if (wizard.currentStep === 1) {
                        return;
                    }

                    var step = element.find('.wizard_step').eq(0);
            ...
            handleTarget: function() {
                ...
                element.find('.wizard_form-step-current').removeClass('wizard_form-step-current');
                target.addClass('wizard_form-step-current');
            },
